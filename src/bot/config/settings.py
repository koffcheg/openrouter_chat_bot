...
class Settings(BaseSettings):
    ...
    openrouter_default_model: str = Field(
        default=DEFAULT_CHAT_MODEL, alias="OPENROUTER_DEFAULT_MODEL"
    )
    ...
    database_url: str = Field(
        default="sqlite+aiosqlite:////app/data/bot.db", alias="DATABASE_URL"
    )
    ...
    default_user_cooldown_seconds: int = Field(
        default=20, alias="DEFAULT_USER_COOLDOWN_SECONDS"
    )
    ...
    default_chat_burst_window_seconds: int = Field(
        default=60, alias="DEFAULT_CHAT_BURST_WINDOW_SECONDS"
    )
    ...
    polling_allowed_updates: str = Field(
        default="message,edited_message", alias="POLLING_ALLOWED_UPDATES"
    )
    polling_drop_pending_updates: bool = Field(
        default=False, alias="POLLING_DROP_PENDING_UPDATES"
    )

    enable_admin_model_selection: bool = Field(
        default=True, alias="ENABLE_ADMIN_MODEL_SELECTION"
    )
    enable_fallback_routing: bool = Field(default=True, alias="ENABLE_FALLBACK_ROUTING")
    ...
    healthcheck_file: str = Field(default="/tmp/bot_healthy", alias="HEALTHCHECK_FILE")
    status_cache_ttl_seconds: int = Field(default=10, alias="STATUS_CACHE_TTL_SECONDS")
