...
def compat(request, monkeypatch):
    ...
    def _init(self, *, openrouter_client, chat_settings_repository, model_router=None, status_service=None, max_input_chars):
        return orig_init(self, openrouter_client=openrouter_client, chat_settings_repository=chat_settings_repository, model_router=model_router or ModelRouter(ModelRegistry.default()), status_service=status_service or StatusService(), max_input_chars=max_input_chars)
    ...
    if msg_cls and hasattr(msg_cls, '__init__'):
        ...
        def _msg(self, *a, **kw):
            orig_msg(self, *a, **kw)
            if not hasattr(self, 'message_id'): self.message_id = 1
            if not hasattr(self, 'bot'): self.bot = SimpleNamespace(send_chat_action=_noop_send_chat_action)
        monkeypatch.setattr(msg_cls, '__init__', _msg, raising=True)
    orig_cleanup = text_mod.cleanup_model_text
    def _cleanup(text):
        cleaned = orig_cleanup(text)
        cleaned = cleaned.replace('Theclaim','The claim').replace('Почемупрограммисты','Почему программисты')
        return cleaned
